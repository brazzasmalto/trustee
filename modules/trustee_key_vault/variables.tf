...
variable "log_analytics_workspace_id" {
  description = "(Optional) Specifies the log analytics workspace id"
  type        = string
}

variable "log_analytics_retention_days" {
  description = "(Optional) Specifies the number of days of the retention policy"
  type        = number
  default     = 7
}

variable "read_only_principals_object_ids" {
  description = "(Optional) Specifies A list of object IDs that are allowed to read elements in the keyvault"
  type        = list(string)
  default     = []
}
...
variable "key_vault_network_acls" {
  description = "(Required) Specifies a custom object with attributes 'bypass', 'default_action', 'ip_rules', 'virtual_network_subnet_ids'."
  type = object({
    bypass                     = string
...
variable "tags" {
  description = "(Optional) Specifies the tags of the keyvault"
  type        = map(string)
  default     = {}
}